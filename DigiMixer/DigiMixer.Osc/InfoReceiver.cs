...
{
    private readonly ConcurrentDictionary<string, OscMessage> messages;
    ...
    /// </summary>
    internal static Task<IDictionary<string, OscMessage>?> RequestAndWait(IOscClient client, string[] addresses, CancellationToken cancellationToken) =>
        RequestAndWait(client, addresses.Select(addr => new OscMessage(addr)), addresses, cancellationToken);
    ...
    /// </summary>
    internal static async Task<IDictionary<string, OscMessage>?> RequestAndWait(IOscClient client, IEnumerable<OscMessage> messages, string[] addresses, CancellationToken cancellationToken)
    {

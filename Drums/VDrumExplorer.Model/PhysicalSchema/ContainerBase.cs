...
namespace VDrumExplorer.Model.PhysicalSchema
{
    ...
    public abstract class ContainerBase : IContainer
    {
        ...
        private protected ContainerBase(string name, string description, ModuleAddress address, string path) =>
            (Name, Description, Address, Path) = (name, description, address, path);
        ...
        public IContainer ResolveContainer(string path)
        {
            ...
            foreach (var segment in segments)
            {
                if (current is ContainerContainer cc)
                {
                    current = cc.Containers.FirstOrDefault(c => c.Name == segment)
        ...
        public (FieldContainer container, IField field) ResolveField(string path)
        {
            ...
            return container is FieldContainer fc
                ? (fc, fc.Fields.FirstOrDefault(field => field.Name == fieldName) ?? throw new ArgumentException($"No field '{fieldName}' within container '{fc.Path}'"))

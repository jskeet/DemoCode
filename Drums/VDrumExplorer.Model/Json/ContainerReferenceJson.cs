...
namespace VDrumExplorer.Model.Json
{
    ...
    internal class ContainerReferenceJson : ContainerItemBase
    {
        ...
        internal IEnumerable<IContainer> ToContainers(ModuleJson module, ModuleAddress parentAddress, string? parentPath, SchemaVariables variables)
        {
            variables = variables.WithVariables(ExtraVariables);
            ModuleOffset offset = ModuleOffset.FromDisplayValue(Offset.Value);
            if (Repeat is null)
            {
                yield return resolvedContainer.ToContainer(module, ResolvedName, ResolvedDescription,
            ...
            else
            {
                ...
                foreach (var tuple in module.GetRepeatSequence(Repeat.Items, variables))
                {
                    ...
                    yield return resolvedContainer.ToContainer(module, formattedName, formattedDescription,

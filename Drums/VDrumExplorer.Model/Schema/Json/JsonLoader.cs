...
namespace VDrumExplorer.Model.Schema.Json
{
    ...
    public sealed class JsonLoader
    {
        ...
        /// <param name="resourceBase">The base of the resources names, relative to the name of the assembly.
        /// May be empty, for resources at the "root" of the assembly. Otherwise, a dot is appended to this name before appending
        ...
        public static JsonLoader FromAssemblyResources(Assembly assembly, string resourceBase)
        {
            ...
            string fullResourceBase = resourceBase == "" ? assemblyName : $"{assemblyName}.{resourceBase}";
            return new JsonLoader(resourceName =>
            {
                string parent = fullResourceBase;
                while (resourceName.StartsWith("../"))
                {
                    var lastDot = parent.LastIndexOf('.');
                    if (lastDot == -1)
                    {
                        throw new InvalidOperationException($"Cannot use .. on root resource");
                    }
                    resourceName = resourceName.Substring(3);
                    parent = parent.Substring(0, lastDot);
                }
                ...
                string fullResourceName = parent + resourceName;
